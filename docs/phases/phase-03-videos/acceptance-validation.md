...
- **Residual cosmetic warning.** `video-worker.module.integration-spec.ts` leaves Jest printing "Jest did not exit one second after the test run has completed" (the pre-fix baseline did not). Exit codes are unaffected — the suite reports `0` and the full run completes. `jest --detectOpenHandles` attributes **no** handle, and an explicit `worker.close()` + `queue.close()` before `app.close()` did not remove it; a 250 ms teardown drain was tried, did not help, and was removed rather than left in as a sleep that buys nothing. Booting a real BullMQ worker in-process is the cause. Tracked as a cosmetic follow-up, stated rather than hidden.
