...
/**
 * Supertest declares `Response.body` as `any`. These shapes describe what the
 * API actually returns, so the assertions below stay type-checked.
 */
interface ErrorBody {
  statusCode: number;
  error: string;
...
/**
 * `mailService` is a private field on `AuthService`; the confirmation and reset
 * tokens are only observable by intercepting the call the service makes on it.
 */
...
describe('Auth (e2e)', () => {
  ...
  async function captureConfirmationToken(
    ...
    let capturedToken = '';
    jest
      .spyOn(mailServiceOf(app.get(AuthService)), 'sendConfirmationEmail')
  ...
  async function registerConfirmAndLogin(
    ...
    const res = await request(app.getHttpServer())
      .post('/auth/login')
      .send({ email, password });
    return bodyOf<TokenPairBody>(res);
  }
  ...
  describe('POST /auth/register', () => {
    it('returns 201 with { id, email } on valid registration', async () => {
      ...
      expect(bodyOf<RegisterBody>(res).id).toBeDefined();
      expect(bodyOf<RegisterBody>(res).email).toBe('user@example.com');
    });
  ...
  describe('JWT Guard', () => {
    ...
    it('returns 200 on GET /auth/me with a valid access token', async () => {
      ...
      expect(bodyOf<MeBody>(res).sub).toBeDefined();
      expect(bodyOf<MeBody>(res).email).toBe('me@example.com');
    });
  ...
  async function capturePasswordResetToken(email: string): Promise<string> {
    let captured = '';
    jest
      .spyOn(mailServiceOf(app.get(AuthService)), 'sendPasswordResetEmail')
      .mockImplementationOnce((_e: string, _n: string, t: string) => {
        captured = t;
        return Promise.resolve();
      });
